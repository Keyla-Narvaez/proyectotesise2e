...
class PurchaseOrderPage {
    ...
    fillItems() {
        ...
        return cy.wrap(articulos).each((item, index) => {
            ...
            cy.xpath(`//input[contains(@name,'ATp:Lines:${index}:Item')]`, {
                    timeout: 30000
                })
                .should('be.visible')
                .click({
            ...
            cy.wait(3000); // ⚠️ temporal; idealmente reemplazar con intercept
            ...
            if (index < articulos.length - 1) {
                cy.get('#_FOpt1\\:_FOr1\\:0\\:_FONSr2\\:0\\:MAt2\\:0\\:AP1\\:AT1\\:_ATp\\:create')
                    .click();
                cy.wait(3000); // ⚠️ también reemplazable
            }
        });
    }
  ...
  cy.xpath("//span[contains(@class,'x2e0')]", { timeout: 30000 }).should('be.visible');
