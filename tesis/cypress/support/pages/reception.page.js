...
class receptionPage {
  navigateInventory() {
    ...
    /*cy.get('a[title="Recibir envíos previstos"]')
      ...
      .click();
      cy.wait(1000);*/
    cy.get('a[title="Favoritos y Elementos Recientes"]', { timeout: 120000 })
      .should('exist')
      .scrollIntoView()
      .invoke('removeAttr', 'href')
      .click({ force: true });
    ...
    cy.get('a[title="Recibir envíos previstos"]', { timeout: 15000 })
      .should('exist')
      .scrollIntoView()
      .invoke('removeAttr', 'href')
      .click({ force: true });
  ...
  OcReception(codigoOC) {
    cy.then(() => {
      ...
      cy.wrap(articulos.purchaseOrder.articulos).each((item, index) => {
        ...
        // Campo "Localizador" en Oracle Fusion
        cy.get('input[aria-label="Localizador"]', { timeout: 15000 })
          .should('exist')
          .scrollIntoView()
          .should('be.visible')
          .click()
          .then(() => {
            // Oracle suele reconstruir el input al hacer click → usamos focused()
            cy.focused().clear().type('L1.01.01.01');
          });


        const codigoFormateado = `CB2-${item.codigoArticulo.slice(-3)}`;
        cy.get(
        ...
        /* cy.get(`input[id*="table1:${index}"][id*="LotExpid::content"]`, { timeout: 15000 })
   .should('be.visible')
   .click({ force: true })
   .clear({ force: true })
   .type("10/26/30", { force: true });cy.get(`input[id*="table1:${index}"][id*="LotExpid::content"]`, { timeout: 15000 })
   .should('be.visible')
   .click({ force: true })
   .clear({ force: true })
   .type("10/26/30", { force: true });
